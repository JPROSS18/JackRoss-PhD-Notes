...
node_sn.model.drdt = 0.01
sn_analysis_node.model.model.dadt = 0.01

# Setting interval length + other parameters 
final_a = 0.5
dt = 0.001
final_t = int(final_a/sn_analysis_node.model.model.dadt)
...
full_traj = node_sn.solve(x0 = torch.tensor([-1.0, -0.5], dtype=torch.float32), t_span=(0, final_t), dt=dt)
...
torch.save(init_tensor, f"Lyapunov_EWS_project/init_tensor_all_intervals_sn.pt")
'''

init_tensor = torch.load(f"Lyapunov_EWS_project/init_tensor_all_intervals_sn.pt").to(device, dtype=torch.float32)


List_Intervals = torch.arange(1, 5, 1, dtype = torch.float32) 
...
start_time = time.time()
temp_list = []
for i in range(0, List_Intervals.shape[0]-1):
    lle_node = sn_analysis_node.lyapunov_spectrum(x0 =init_tensor.unsqueeze(2), k = 1, t0 = 0, dt = dt,
                                                                num_pts_compute=int(List_Intervals[i]/dt), t_transient_pts=0, non_autonomous=False)
    temp_list.append(lle_node)
    torch.save(lle_node, f"Lyapunov_EWS_project/lle_node_{List_Intervals[i].item():.0f}_interval_sn.pt")
